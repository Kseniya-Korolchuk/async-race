...
export const getDistance = (firstElem: HTMLElement, secondElem: HTMLElement): number => {
    const firstElemPos = getPosition(firstElem);
    const secondElemPos = getPosition(secondElem);
...
export const animation = (car: HTMLElement, distanceBtwElem: number, animationTime: number): { id: number } => {
    ...
    const getStep = (timestamp: number) => {
        ...
        targetCar.style.transform = `translateX(${Math.min(passed, distanceBtwElem)}px) translateY(0px)`;
